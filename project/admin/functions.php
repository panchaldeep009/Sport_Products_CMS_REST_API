<?php
    ...
    function edit_user($username, $email, $name, $password, $pdo){
        ...
        if($update_user->rowCount() > 0){
            if($username != null) { $_SESSION['user']['user_username'] = $username; }
            if($email != null) { $_SESSION['user']['user_email'] = $email; }
            if($name != null) { $_SESSION['user']['user_name'] = $name; }
